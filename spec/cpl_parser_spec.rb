...
describe BookWorm::CplParser do
  ...
  it "should parse fines" do
    p = described_class.new(:all => File.read(File.join(fixture_path, "just_fines.html")))
    p.total_fine_amount.should == 10 
  end

  it 'should get the correct number of held_items' do
    p = described_class.new(:all => File.read(File.join(fixture_path, 'checked_out_overdue_on_hold.html')))
